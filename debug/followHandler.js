...
$.consoleLn(">>> followHandler.js: twitchFollow(event)");
    if (!$.bot.isModuleEnabled('./handlers/followHandler.js')) {
$.consoleLn(">>> followHandler.js: module is disabled.");
       return;
    }

    if (!followToggle) {
$.consoleLn(">>> followHandler.js: toggle is disabled.");
        return;
    }

    var follower = event.getFollower().toLowerCase();
$.consoleLn(">>> followHandler.js: got follower: " + follower);

    if ($.inidb.exists('followed', follower)) {
$.consoleLn(">>> followHandler.js: follower already exists in followed table.");
       return;
    }
    ...
    if (announceFollows) {
$.consoleLn(">>> followHandler.js: announceFollows");
...
$.consoleLn(">>> followHandler.js: followmsg (reward) = " + followmsg);
      } else {
        followmsg = followMessageNoReward.replace('(name)', $.username.resolve(follower));
$.consoleLn(">>> followHandler.js: followmsg = " + followmsg);
      }
      $.say(followmsg);
    }
