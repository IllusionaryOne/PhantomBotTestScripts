...
(function() {
    ...
    $.bind('ircChannelJoin', function(event) {
        if ($.isOnline($.channelName)) {
            ...
            $.consoleLn('greetingSystem: user: ' + sender + ' lastUserGreeting: ' + lastUserGreeting + ' greetingCooldown: ' + greetingCooldown + ' sum: ' + (lastUserGreeting + greetingCooldown) + ' < now: ' + now);
            if (lastUserGreeting + greetingCooldown < now) {
                $.consoleLn('greetingSystem: lastUserGreeting + greetingCooldown < now, proceeding');
                if (message) {
                    $.consoleLn('greetingSystem: found message: ' + message);
                    $.say(message);
